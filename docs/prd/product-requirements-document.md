...
- Runtime: rootful Podman pod, shared `net,ipc,uts` namespaces and **host cgroup namespace (`--cgroupns=host`)** so the programs can attach at the pod common parent cgroup; the sidecar container runs entirely as UID 1337 (loop exclusion); sidecar caps after `--cap-drop ALL`: `CAP_BPF` + `CAP_NET_ADMIN` (redirect) + `CAP_SYS_RESOURCE` (BPF memlock) + `CAP_NET_RAW` (capture socket); the app container is launched with `LD_PRELOAD=<interposer.so>` for TLS key extraction (no shared PID namespace, no `CAP_PERFMON` — both were uprobe-attach-only and are no longer needed); kernel ≥ 5.10; mounts `/sys/fs/bpf`, `/sys/fs/cgroup`.

- Supported platforms: local development runs on rootful Podman. Target platforms for Kubernetes worker nodes, in priority order (AD-013): **Ubuntu 24.04 LTS** (→ 26.04 LTS) as the reference platform that gates release, then Bottlerocket, Flatcar Container Linux, Talos Linux, Fedora CoreOS and Rocky Linux. No platform is verified yet; per-distro validation is tracked as the `04-platform-support` feature. (Production Kubernetes itself remains an MVP non-goal — see below.)
