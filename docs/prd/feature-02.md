...
- The Go sidecar reads newline-delimited lines from the socket and runs each one through the **existing, unchanged** NSS line validator, dedup, and secure append writer (`internal/keylog/nss.go`, `writer.go`) — appending to `/var/log/sidecar/sslkeylog.log`, deduplicated by `(label, client_random, secret)`.
...
- The interposer is a small, dependency-free C shared library, built with `clang`/`gcc` (the project's existing C toolchain, already used for the eBPF programs) — not linked into or built as part of the Go binary; the main sidecar binary remains CGO-free.
